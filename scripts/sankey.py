...
def process(ref_df_, snodb_df, sno_host_df, sno_host_loc_df, data_df):
    ...
    NETWORK_SNO = set(data_df.single_id1)
    ...
    OTHER_NET_SNO = NETWORK_SNO - INTRA_NET_SNO
...
def sankey_graph(groups, values):
    ...
    sankey.add(
        flows=[values[8], -values[10], -values[9]],
        orientations=[0, 1, -1],
        pathlengths=[-.3, 0.5, 0.5],
...
def main():
    ...
    groups, values = process(ref_df, snodb_df, sno_host_df, sno_host_loc_df, data_df)
