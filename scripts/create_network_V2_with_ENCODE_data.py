...
def create_sif(df_, id_name_dict):
    ...
    df = df[[
        ...
        'name1', 'name2', 'biotype2', 'host_interaction',
    ...
    for col in df.values:
        chr, start, end, sup, DG, id1, id2, name1, name2, bio2, h_i, E, p_i, merged_id = col
        pro_edge = 'pr'
        ...
        if prot_info != 'nan':
            for prot_list in prot_info.replace(('['), '').replace(']', '').replace('),', ')),').split('), '):
                ...
                tmp = (id_name_dict[prot], prot, pro_edge, id1, name1, sno_score, sno_pValue, DG)
                single_int_clip.append(tmp)

                # Add value fot the target
                tmp = (id_name_dict[prot], prot, pro_edge, id2,
                       name2, target_score, target_pValue, DG)
                single_int_clip.append(tmp)
...
def create_node_info(df, clip_df):
...
def main():
    ...
    create_node_info(base_df, clip_df)
